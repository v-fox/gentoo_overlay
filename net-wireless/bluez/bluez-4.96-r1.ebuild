...
inherit multilib eutils systemd autotools
...
IUSE="alsa caps +consolekit cups debug gstreamer maemo6 +health old-daemons pcmcia pnat +proximity +sap sixaxis +temperature test-programs +usb wii"
...
src_configure() {
	econf \
		...
		$(use_enable pcmcia) \
		$(use_enable pnat) \
		$(use_enable proximity) \
		$(use_enable sap) \
		$(use_enable sixaxis) \
		$(use_enable temperature thermometer) \
		$(use_enable test-programs test) \
		$(use_enable usb) \
		$(use_enable wii wiimote)
}
