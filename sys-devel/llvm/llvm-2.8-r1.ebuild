...
IUSE="alltargets debug docs +libffi llvm-gcc ocaml test +udis86 +threads"
...
src_configure() {
	local CONF_FLAGS="--enable-shared --enable-jit"
	...
	if use alltargets; then
		CONF_FLAGS+=" --enable-targets=all"
	else
		use multilib && \
			CONF_FLAGS+=" --enable-targets=x86,x86_64" || \
			CONF_FLAGS+=" --enable-targets=host-only"
	fi

	if use amd64; then
		CONF_FLAGS+=" --enable-pic"
	fi
	...
	if use ocaml; then
		CONF_FLAGS+=" --enable-bindings=ocaml"
	else
		CONF_FLAGS+=" --enable-bindings=auto"
	fi

	CONF_FLAGS="${CONF_FLAGS}
		$(use_enable docs doxygen)
		$(use_enable libffi)
		$(use_enable threads)
		$(use_with udis86)"
