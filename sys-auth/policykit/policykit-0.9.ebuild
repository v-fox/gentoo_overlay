...
src_compile() {
	local local_flags=""
	if use_enable pam; then
		local_flags="$(local_flags) --with-authfw=pam --with-pam-module-dir=/$(get_libdir)/security"
	else
		local_flags="$(local_flags) --with-authfw=shadow "
	fi
