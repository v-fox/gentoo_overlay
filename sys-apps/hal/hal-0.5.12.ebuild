...
src_prepare() {
	cp -a "${FILESDIR}/${PV}" "${WORKDIR}"/

	# Only apply one of the policy patches.  Bug #267042
#	if use policykit ; then
#		rm "${WORKDIR}/${PV}/0001-plugdev-dbus-policy.patch"
#	else
#		rm "${WORKDIR}/${PV}/0002-policykit-dbus-policy.patch"
#	fi
	rm "${WORKDIR}/${PV}"/000{1,2}*.patch || die
	EPATCH_MULTI_MSG="Applying Gentoo Patchset ..." \
	EPATCH_SUFFIX="patch" \
	EPATCH_SOURCE="${WORKDIR}/${PV}/" \
	EPATCH_FORCE="yes" \
	epatch
