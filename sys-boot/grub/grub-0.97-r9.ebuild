...
pkg_setup() {
	...
	if use amd64 && ! use multilib; then
		eerror "You are not using multilib profile and"
		eerror "GRUB does not work if compiled in 64bit"
		eerror "so, change profile or forget about it"
		epause 5
		die "not suitable profile"
	fi
}
