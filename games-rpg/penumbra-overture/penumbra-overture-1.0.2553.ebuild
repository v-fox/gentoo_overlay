...
pkg_nofetch() {
	...
	einfo
	einfo "Also http://www.getpenumbra.com/downloads/updates/ep1/linux/PenumbraOvertureUpgrade-${PV}.sh"
	echo
}
...
src_unpack() {
	...
	local f
	for f in PenumbraOverture{-${FIRST_PV},Upgrade-${PV}}.sh ; do
		cd "${WORKDIR}"
		unpack_makeself "${f}"
