...
src_unpack() {
	...
	cp "${DISTDIR}"/glext.h "${WORKDIR}"/
	cp "${DISTDIR}"/glxext.h "${WORKDIR}"/
	cp "${DISTDIR}"/wglext.h "${WORKDIR}"/

	sed -i -e 's:glxext.h:glxext.h wglext.h:' opengl.eselect || die
}
